    ...
    .then(function (body) {
      var data = body.data;
        ...
        sql = sql.replace('#id#', photo.id)
                  ...
                  .replace(/#caption#/g, mysqlStringScrub(photo.caption ? photo.caption.text : ""));
        ...
        if (bigSqlStr !== "") {
          mysqlClient.query(userSql + bigSqlStr, function (rows) {
            ...
            log.info(("Processed " + 1 + " bags").green);
...
function processBags(url, tag) {
  ...
  instagramFetch(url)
    .then(function (body) {
      ...
      if (!body.data.length && body.data !== undefined) {
        body.data = [body.data];
      }
      if (!body.data.length) {
